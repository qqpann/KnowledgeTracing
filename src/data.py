    ...
    if source == SOURCE_ASSIST0910_SELF:
        ...
        with open(trainfname, 'r') as f:
            ...
            for idx in range(0, len(lines), 3):
                qlist = list(
                    map(int, lines[idx + 1].strip().rstrip(',').split(',')))
                alist = list(
                    map(int, lines[idx + 2].strip().rstrip(',').split(',')))
...
class QandAEmbedder:
    ...
    def qaToIdxNum(self, q_and_a: Tuple) -> int:
        ...
        if not (0 <= q_and_a[0] < self.M):
            raise ValueError(
                'q out of range. Got {0} but expected 0<=q<M={1}'.format(q_and_a, self.M))
        elif (q_and_a[1] not in {0, 1}):
            raise ValueError('a out of range')

        # 0: PAD, 1: SOS
        # consider 0:pad and 1:sos
        return q_and_a[0] + q_and_a[1] * self.M + self.PT
...
def slice_d(d: List, x_seq_size: int, type: str = 'base', sliding_window: int = 0, reverse: bool = False, extend_backward: int = 0, extend_forward: int = 0) -> Tuple[List]:
    ...
    max_iter = min(
        max_iter, sliding_window) if sliding_window != 0 else max_iter
    ...
    for i in range(0, direction * max_iter, direction * 1):
        result.append(
            d[prefix + i * seq_size: prefix + i * seq_size + seq_size])
    return result
